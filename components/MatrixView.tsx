...
import { PfaRecord, TimelineBounds, GroupingField, SeriesVisibility, DisplayMetric, GroupNode, GridColumn, AssetMasterRecord, ClassificationRecord } from '../types';
import { getMonthLabels, groupAssets, formatDate, formatCurrencyCompact } from '../utils';
...
const CompactQuantityMatrix = ({ assets, bounds, months, onInitiateAddForecast, gridColumns, visibleSeries }: { 
    assets: PfaRecord[], 
    bounds: TimelineBounds, 
    months: Date[],
...
export const MatrixView: React.FC<MatrixViewProps> = ({
  ...
  const hierarchicalData = useMemo(() => {
      ...
      return groupAssets(assets, grouping);
  }, [assets, grouping, barDisplayMode]);
  ...
  const renderMatrixGroup = (node: GroupNode) => {
      ...
      return (
          <div key={node.id}>
              ...
              {!isCollapsed && (
                  <>
                     {node.children.map(child => renderMatrixGroup(child))}
                     {showAssets && node.assets.map(asset => {
                         return (
                                ...
                                <div className="flex-1 flex flex-col">
                                    ...
                                    {visibleSeries.plan && (
                                        <div className="flex h-8 hover:bg-slate-50 dark:hover:bg-slate-800/50 items-center">
                                                ...
                                                <div className="flex">
                                                {months.map((m, i) => {
                                                    const active = isInRange(m, asset.originalStart, asset.originalEnd);
                                                    return (
                                    ...
                                    {visibleSeries.forecast && (
                                            ...
                                            <div className="flex-none w-10 flex items-center justify-center border-r border-slate-100 dark:border-slate-800 bg-blue-50/50 dark:bg-blue-900/10">
                                                ...
                                                <div className="flex">
                                                {months.map((m, i) => {
                                                    const active = isInRange(m, asset.forecastStart, asset.forecastEnd);
                                                    return (
                                    ...
                                    {visibleSeries.actual && asset.hasActuals && (
                                            ...
                                            <div className="flex-none w-10 flex items-center justify-center border-r border-slate-100 dark:border-slate-800 bg-emerald-50/50 dark:bg-emerald-900/10">
                                                ...
                                                <div className="flex">
                                                {months.map((m, i) => {
                                                    const active = isInRange(m, asset.actualStart, asset.actualEnd);
                                                    return (
