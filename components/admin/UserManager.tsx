...
import { Users, Plus, Shield, User as UserIcon, Edit2, Trash2, Search, Briefcase, RefreshCw } from 'lucide-react';
  ...
  const handleSave = async (userData: Partial<User>, password?: string) => {
      ...
      try {
          const payload: any = {
              ...
              organizationIds: userData.allowedOrganizationIds,
          };
          ...
          if (userData.id) {
              ...
              await apiClient.updateUser(userData.id, payload);
              await refreshUsers(); // Refresh to get updated data
          } else {
              // Create new user
              payload.password = password || 'changeme123';
              await apiClient.createUser(payload);
              await refreshUsers(); // Refresh to get new user
          }
  ...
  const handleDelete = async (id: string) => {
      if (window.confirm("Are you sure you want to delete this user?")) {
          try {
              ...
              await refreshUsers(); // Refresh after deletion
          } catch (error) {
    ...
    <div className="space-y-6">
         <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
            <div>
                ...
                <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Manage system access, roles, and user profiles.</p>
            </div>
            ...
            <div className="flex items-center gap-3 w-full md:w-auto">
                ...
                <button
                    ...
                    className="flex-none flex items-center gap-2 px-4 py-2 bg-slate-100 hover:bg-slate-200 dark:bg-slate-700 dark:hover:bg-slate-600 text-slate-700 dark:text-slate-200 rounded-lg font-medium transition-colors disabled:opacity-50"
        ...
        <div className="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 overflow-hidden">
            <table className="w-full">
                <thead className="bg-gray-50 dark:bg-gray-900 border-b border-gray-200 dark:border-gray-700">
                    <tr>
                        <th className="px-6 py-3 text-left text-xs font-semibold text-gray-600 dark:text-gray-300 uppercase tracking-wider">User Identity</th>
                        <th className="px-6 py-3 text-left text-xs font-semibold text-gray-600 dark:text-gray-300 uppercase tracking-wider">Role</th>
                        <th className="px-6 py-3 text-left text-xs font-semibold text-gray-600 dark:text-gray-300 uppercase tracking-wider">Organizations</th>
                ...
                <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
                    {filteredUsers.map(user => (
                        ...
                        >
                            ...
                            <td className="px-6 py-4">
                                <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium border ${user.role === 'admin' ? 'bg-purple-100 text-purple-700 border-purple-200 dark:bg-purple-900/20 dark:text-purple-400 dark:border-purple-800' : 'bg-blue-50 text-blue-600 border-blue-200 dark:bg-blue-900/20 dark:text-blue-400 dark:border-blue-800'}`}>
