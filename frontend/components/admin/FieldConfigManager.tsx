/**
 * FieldConfigManager Component
 * Phase 5 - Field Configuration
 *
 * Manages field mappings for import/export operations.
...
export const FieldConfigManager: React.FC<FieldConfigManagerProps> = ({
  config,
  setConfig,
}) => {
  ...
  const updateField = (id: string, updates: Partial<FieldDefinition>) => {
    const updatedList = activeList.map((f) =>
      f.id === id ? { ...f, ...updates } : f
    );
    setConfig({
      ...config,
        ...
        <div className="flex-1 overflow-y-auto py-2">
          ...
          {CATEGORIES.filter((c) => c.type === 'export').map((cat) => (
            <button
              ...
              className={`w-full text-left px-4 py-2 text-sm font-medium border-l-4 transition-colors ${
      ...
      <div className="flex-1 flex flex-col h-full overflow-hidden">
        <div className="p-6 border-b border-slate-700 bg-slate-800/30">
          <div className="flex items-center gap-2 mb-4">
            {activeCategory.includes('import') ? (
          ...
          <div className="grid grid-cols-3 gap-2">
            <input
              ...
              className="px-3 py-2 bg-slate-900 border border-slate-600 rounded-lg text-sm text-slate-100 placeholder-slate-500 focus:outline-none focus:border-blue-500"
            />
            <div className="relative">
              ...
              <input
                ...
                className="w-full pl-8 pr-3 py-2 bg-slate-900 border border-slate-600 rounded-lg text-sm text-slate-100 placeholder-slate-500 focus:outline-none focus:border-blue-500 font-mono"
              />
            </div>
            <div className="flex gap-2">
              <div className="relative flex-1">
                ...
                <select
                  ...
                  className="w-full pl-8 pr-3 py-2 bg-slate-900 border border-slate-600 rounded-lg text-sm text-slate-100 focus:outline-none focus:border-blue-500 appearance-none cursor-pointer"
                >
              ...
              <button
                ...
                className="px-4 py-2 bg-blue-600 hover:bg-blue-500 disabled:bg-slate-600 disabled:cursor-not-allowed text-white rounded-lg font-bold text-sm flex items-center gap-2 transition-colors flex-none"
              >
        ...
        <div className="flex-1 overflow-y-auto p-6 custom-scrollbar">
          <div className="space-y-2">
            {/* Header Row */}
            <div className="grid grid-cols-12 gap-4 px-3 py-2 text-[10px] font-bold text-slate-500 uppercase tracking-wider">
              ...
              >
                ...
                <div className="col-span-3">
                  <div
                    className={`text-sm font-bold ${
                      field.enabled ? 'text-slate-100' : 'text-slate-500'
                    }`}
                  >
                    {field.label}
                  </div>
                ...
                <div className="col-span-4">
                  <div className="flex items-center gap-2 bg-slate-900 rounded px-2 py-1">
                    <Link className="w-3 h-3 text-slate-500 flex-none" />
                    <input
                      type="text"
                      value={field.apiMap || ''}
                      onChange={(e) =>
                        updateField(field.id, { apiMap: e.target.value })
                      }
                      placeholder="Unmapped"
                      className="bg-transparent border-none focus:ring-0 p-0 w-full text-xs font-mono text-slate-300 focus:outline-none placeholder-slate-600"
                    />
                  </div>
                </div>
                ...
                <div className="col-span-3">
                  <div className="flex items-center gap-2 px-2 py-1">
                    <Database className="w-3 h-3 text-slate-500 flex-none" />
                    <select
                      value={field.internalKey || ''}
                      onChange={(e) =>
                        updateField(field.id, { internalKey: e.target.value as any })
                      }
                      className="bg-transparent border-b border-slate-700 focus:border-blue-500 text-xs text-slate-300 w-full outline-none p-0 appearance-none cursor-pointer hover:text-blue-400"
                    >
                ...
                <div className="col-span-1 text-right">
                  {field.isCustom ? (
                    ...
                    <span className="text-[10px] text-slate-500 font-medium px-2 bg-slate-700 rounded">
                      Sys
                    </span>
                  )}
                </div>
