...
import { Database, RefreshCw, CheckCircle, XCircle, Loader2, AlertTriangle, Play, History } from 'lucide-react';
...
export const PemsSync: React.FC = () => {
    ...
    if (activeSyncId && activeSyncStatus?.status === 'in_progress') {
      const interval = setInterval(async () => {
        try {
          ...
          setActiveSyncStatus(status as unknown as SyncStatus);
  ...
  const handleSync = async (syncType: 'full' | 'incremental' = 'full') => {
    ...
    if (!readConfig || !readConfig.id) {
      alert('No read API configuration found');
      return;
    }

    try {
      setIsSyncing(true);
      const result = await apiClient.syncPemsData(currentOrganizationId, readConfig.id, syncType);
