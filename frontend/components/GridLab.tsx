...
export const GridLab: React.FC<GridLabProps> = ({ 
  ...
  const sortedAssets = useMemo(() => {
    // Assets are already filtered by the parent (Cockpit/FilterPanel)
    const result = [...assets];
  ...
  return (
    <div className="flex flex-col h-full bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-700 overflow-hidden shadow-xl shadow-slate-200/50 dark:shadow-black/50 select-none relative">
      ...
      <div className="flex-1 overflow-auto custom-scrollbar relative" ref={scrollRef} onScroll={handleScroll}>
          ...
          <table className="w-full whitespace-nowrap absolute top-0 left-0 z-20 bg-slate-100 dark:bg-slate-800 shadow-sm">
            <thead>
              <tr>
                ...
                <Th label="Sync State" sKey="id" width="110px" />
                ...
                <Th label="Rate/Price" sKey="monthlyRate" width="120px" />
          ...
          {/* Virtual Content */}
          <div style={{ height: totalHeight, paddingTop: 45 }} className="relative"> {/* 45px header buffer */}
             <table className="w-full whitespace-nowrap absolute top-0 left-0" style={{ transform: `translateY(${startOffset + 45}px)` }}>
                 {/* Match header layout implicitly via table layout */}
                 <colgroup>
                    <col style={{width: 40}} />
                    <col style={{width: 120}} />
                    <col style={{width: 110}} />
                  ...
                  {visibleAssets.map((asset) => {
                    ...
                    return (
                        ...
                        <td className="px-4 py-3 text-xs">
                            {(() => {
                                const syncState = (asset as any)._metadata?.syncState || 'pristine';
                                const colors = {
                                    pristine: 'bg-slate-100 border-slate-200 text-slate-600 dark:bg-slate-800 dark:border-slate-700 dark:text-slate-400',
                                    draft: 'bg-yellow-100 border-yellow-300 text-yellow-700 dark:bg-yellow-900/20 dark:border-yellow-700 dark:text-yellow-400',
                                    committed: 'bg-blue-100 border-blue-300 text-blue-700 dark:bg-blue-900/20 dark:border-blue-700 dark:text-blue-400',
                                    syncing: 'bg-purple-100 border-purple-300 text-purple-700 dark:bg-purple-900/20 dark:border-purple-700 dark:text-purple-400 animate-pulse',
                                    sync_error: 'bg-red-100 border-red-300 text-red-700 dark:bg-red-900/20 dark:border-red-700 dark:text-red-400'
                                };
                                return (
                                    <span className={`px-2 py-0.5 rounded border text-[10px] font-bold ${colors[syncState as keyof typeof colors] || colors.pristine}`}>
                                        {syncState === 'pristine' ? '—' : syncState.replace('_', ' ').toUpperCase()}
                                    </span>
                                );
                            })()}
                        </td>
      ...
      <div className="p-3 border-t border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 text-xs text-slate-500 text-right">
           Showing {sortedAssets.length} records. {selectedIds.size} selected.
      </div>
    </div>
  );
};
