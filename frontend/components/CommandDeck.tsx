...
export const CommandDeck: React.FC<CommandDeckProps> = ({
  ...
  return (
    <div className="fixed bottom-8 left-1/2 transform -translate-x-1/2 z-50 flex flex-col items-center gap-2 animate-in slide-in-from-bottom-4 duration-300">
      ...
      {showEditMenu && !hasActualizedSelected && !isReadOnly && (
      ...
      <div className="bg-slate-900/90 backdrop-blur-md text-white rounded-2xl shadow-2xl border border-slate-700/50 p-2 flex items-center gap-4 shadow-blue-900/20">
        ...
        {!hasActualizedSelected && (
            <>
                <div className="flex flex-col gap-1 px-2">
                    <div className="text-[10px] text-slate-400 font-medium text-center uppercase flex items-center justify-center gap-1">
                        Shift Timeline
                        {!hasPermission('perm_EditForecast') && <Lock className="w-2.5 h-2.5" />}
                    </div>
                    <div className="flex items-center gap-1">
                        ...
                        <button
                            onClick={() => hasPermission('perm_EditForecast') && onShiftTime(-baseStep)}
                            disabled={!hasPermission('perm_EditForecast')}
                            className={`px-2 py-1 text-xs rounded border border-slate-700 transition-colors min-w-[40px] ${hasPermission('perm_EditForecast') ? 'bg-slate-800 hover:bg-slate-700' : 'bg-slate-900 opacity-50 cursor-not-allowed'}`}
                            title={!hasPermission('perm_EditForecast') ? 'Requires Edit Forecast permission' : undefined}
                        >
                            -1{unitLabel}
                        </button>
                        ...
                        <button
                            ...
                            disabled={!hasPermission('perm_EditForecast')}
                            className={`p-2 rounded-md transition-colors ${hasPermission('perm_EditForecast') ? 'hover:bg-white/10 text-slate-300 hover:text-white' : 'opacity-50 cursor-not-allowed text-slate-500'}`}
                            title={hasPermission('perm_EditForecast') ? `Forward ${largeLabelMult} ${scale}s` : 'Requires Edit Forecast permission'}
                        >
        ...
        <div className="flex flex-col gap-1 px-2">
            <div className="text-[10px] text-slate-400 font-medium text-center uppercase flex items-center justify-center gap-1">
                Duration ({scale})
                {!(hasActualizedSelected ? hasPermission('perm_EditActuals') : hasPermission('perm_EditForecast')) && <Lock className="w-2.5 h-2.5" />}
            </div>
            <div className="flex items-center gap-2 justify-center">
                <button
                    ...
                    title={(hasActualizedSelected ? hasPermission('perm_EditActuals') : hasPermission('perm_EditForecast')) ? `Reduce by 1 ${scale}` : `Requires ${hasActualizedSelected ? 'Edit Actuals' : 'Edit Forecast'} permission`}
                >
        ...
        <div className="pl-2 border-l border-slate-700 flex items-center gap-2">
             <PermissionButton
                permission={hasActualizedSelected ? 'perm_EditActuals' : 'perm_EditForecast'}
                ...
                tooltipMessage={`Requires ${hasActualizedSelected ? 'Edit Actuals' : 'Edit Forecast'} permission to reset`}
                showLockIcon={false}
             >
