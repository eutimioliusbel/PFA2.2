...
import { AdminDashboard } from './components/AdminDashboard'; // Consolidate Admin Views
...
import { PanelLeftOpen, Plus, Undo2, Redo2, Check, Trash2, AlertTriangle, RefreshCw, Loader2, Sparkles, Mic, X, Save, Brain, Send, Activity, FileSpreadsheet, Database, FileDown, PanelLeftClose, LogOut, Shield, LayoutGrid, Table, Building2, Plug, Settings, Monitor, Tag, Layers, FileText, Users, Blocks } from 'lucide-react';
import { getTimelineBounds, getDaysDiff } from './utils';

// --- Missing Definitions Restored ---

type AppMode = 'timeline-lab' | 'matrix-lab' | 'grid-lab' | 'pfa-1.0-lab' | 'export' | 'admin-dashboard';
  ...
  if (appMode === 'admin-dashboard') {
      return (
          <AdminDashboard 
              ...
              onLaunchLab={() => setAppMode('timeline-lab')}
              onOpenProfile={() => setShowProfile(true)}
              users={users} setUsers={setUsers}
              orgs={orgs} setOrgs={setOrgs}
              apis={apis} setApis={setApis}
              systemConfig={systemConfig} setSystemConfig={setSystemConfig}
              exportConfig={exportConfig} setExportConfig={setExportConfig}
                ...
                <div className="flex-1 overflow-y-auto py-4 px-3 space-y-1 custom-scrollbar">
                    ...
                    {currentUser.role === 'admin' && (
                        <>
                            ...
                            <MenuItem label="Admin Console" icon={Settings} active={appMode === 'admin-dashboard'} onClick={() => setAppMode('admin-dashboard')} />
                        </>
                    )}
                </div>
