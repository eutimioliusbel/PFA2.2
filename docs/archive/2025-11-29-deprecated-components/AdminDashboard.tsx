...
import { NLQueryInput } from './admin/NLQueryInput';
import { RoleTemplateEditor } from './admin/RoleTemplateEditor';
import { TrashCan } from './admin/TrashCan';
import { PersonalAccessTokens } from './admin/PersonalAccessTokens';
import { SessionManager } from './admin/SessionManager';
import { IntegrationsHub } from './admin/IntegrationsHub';
import { SystemDictionaryEditor } from './admin/SystemDictionaryEditor';
import { SyncHistoryDashboard } from './admin/SyncHistoryDashboard';
import { Users, Building2, Plug, LogOut, Settings, Activity, FileSpreadsheet, FileText, User as UserIcon, Database, FileDown, PanelLeftClose, PanelLeftOpen, Monitor, Tag, Table, Layers, Truck, Factory, Box, History, FileUp, Search, Shield, Trash2, Key, Webhook, BookOpen, RefreshCw } from 'lucide-react';
...
type AdminView = 'users' | 'orgs' | 'apis' | 'system' | 'import' | 'export' | 'asset_master' | 'class_master' | 'pfa_master' | 'dors' | 'sources' | 'manufacturers' | 'models' | 'data_sources' | 'history' | 'ai_import' | 'nl_query' | 'role_templates' | 'trash' | 'tokens' | 'sessions' | 'integrations' | 'dictionary' | 'sync_status';
            ...
            <div className="w-64 flex flex-col h-full">
                ...
                <div className="flex-1 overflow-y-auto py-6 px-3 space-y-8 custom-scrollbar">
                    ...
                    <div className="space-y-1">
                        ...
                        <div className="px-3 mt-6 mb-2 text-[10px] font-bold text-slate-500 uppercase tracking-wider">Security & Access</div>

                        <MenuItem
                            label="Role Templates"
                            icon={Shield}
                            active={activeView === 'role_templates'}
                            onClick={() => setActiveView('role_templates')}
                        />
                        <MenuItem
                            label="Access Tokens"
                            icon={Key}
                        ...
                        />
                        <MenuItem
                            label="Intelligent Import"
                            icon={FileUp}
                            active={activeView === 'ai_import'}
                            onClick={() => setActiveView('ai_import')}
                        />
                        <MenuItem
        ...
        <div className="flex-1 flex flex-col h-screen overflow-hidden bg-slate-50 dark:bg-slate-950 relative">
            ...
            <main className="flex-1 overflow-y-auto p-8 custom-scrollbar">
                <div className="max-w-[95%] mx-auto">
                    ...
                    {activeView === 'nl_query' && <NLQueryInput />}
                    {activeView === 'apis' && <ApiConnectivity />}
                    {activeView === 'data_sources' && <DataSourceManager />}
                    {activeView === 'history' && <HistoryTab organizationId={currentUser.organizationId} currentUser={currentUser} />}
                    ...
                    {activeView === 'ai_import' && <ImportWizard organizationId={currentUser.organizationId} />}
